...
`LocalEmbeddingProvider` performs real Live inference inside the trusted deployment for both queries and items. This permits note-bearing semantic input without sending private notes to external AI providers under `L2-041`. The configured local model/version is identical on both paths; Demo remains a separate explicitly labeled implementation.
...
`SemanticCursor` binds generation, query/filter fingerprint, page size, last score, and identifier. A changed generation produces 409 with Refresh required rather than mixing pages. A request that changes query or filters starts a new traversal. Embedding/search failure exposes Meaning unavailable and an explicit switch to Keyword. Demo output is labeled by execution mode and never presented as live similarity.
