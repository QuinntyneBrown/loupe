...
export class PhotographUpload {
  ...
  readonly errors = computed(() => {
    ...
    return {
      ...
      requestedFeedback: length(this.requestedFeedback()) > 2000,
    };
  });
  readonly fileError = computed(() => {
    if (this.multipleFiles()) return 'Choose one photograph at a time.';
      ...
      const photo = await this.service.upload(
        {
          image,
          title: this.title(),
          brief: { ...this.brief(), requestedFeedback: this.requestedFeedback() },
