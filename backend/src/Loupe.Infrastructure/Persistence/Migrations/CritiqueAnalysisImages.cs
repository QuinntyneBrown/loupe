...
namespace Loupe.Infrastructure.Persistence.Migrations
{
    ...
    [Migration("20260908011430_CritiqueAnalysisImages")]
    public class CritiqueAnalysisImages : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("""
                UPDATE background_operations operation
                SET "InputJson" = jsonb_set(operation."InputJson", '{PreviewKey}', to_jsonb(photograph."PreviewKey"))
                FROM photographs photograph
                WHERE operation."Type" = 'Critique' AND operation."InputJson" IS NOT NULL
                  AND operation."OwnerId" = photograph."OwnerId" AND operation."ResourceId" = photograph."Id"
                  AND operation."InputJson" ->> 'ImageKey' = photograph."ImageKey"
                  AND NOT (operation."InputJson" ? 'PreviewKey')
                """);
        }
        ...
        {
            migrationBuilder.Sql("UPDATE background_operations SET \"InputJson\" = \"InputJson\" - 'PreviewKey' WHERE \"Type\" = 'Critique' AND \"InputJson\" IS NOT NULL");
        }
        /// <inheritdoc />
