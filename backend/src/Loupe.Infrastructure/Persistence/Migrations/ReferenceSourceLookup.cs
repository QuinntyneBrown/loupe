...
namespace Loupe.Infrastructure.Persistence.Migrations
{
    ...
    [Migration("20260908061754_ReferenceSourceLookup")]
    public class ReferenceSourceLookup : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("""
                CREATE FUNCTION loupe_normalize_source(value text) RETURNS text
                LANGUAGE sql IMMUTABLE STRICT AS $$
                  SELECT lower(parts[1]) || '://' ||
                    regexp_replace(lower(parts[2]), CASE WHEN lower(parts[1]) = 'https' THEN ':0*443$' ELSE ':0*80$' END, '') ||
                    CASE WHEN parts[3] = '' OR left(parts[3], 1) = '?' THEN '/' ELSE '' END || parts[3]
                  FROM regexp_match(value, '^([^:]+)://([^/?#]+)([^#]*)') AS source(parts)
                $$;
                """);
            migrationBuilder.AddColumn<string>(
                ...
                maxLength: 32,
                nullable: true,
                computedColumnSql: "md5(loupe_normalize_source(\"SourceUrl\"))",
                stored: true);

            migrationBuilder.CreateIndex(
                name: "IX_references_OwnerId_SourceHash",
                table: "references",
                columns: new[] { "OwnerId", "SourceHash" });
        }
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.DropColumn(
                name: "SourceHash",
                table: "references");
            migrationBuilder.Sql("DROP FUNCTION loupe_normalize_source(text);");
        }
        /// <inheritdoc />
