...
public sealed class AzureOpenAiReferenceAnalysisProvider(IHttpClientFactory clients, IImageStore images, IOptions<AiOptions> options, TimeProvider clock) : IReferenceAnalysisProvider
{
    ...
    public async Task<ReferenceAnalysisResult> GenerateAsync(ReferenceAnalysisInput input, AnalysisIdentity identity, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            request.Content = JsonContent.Create(new
            {
                ...
                instructions = ReferenceAnalysisPrompt.Instructions,
                input = new[] { new { role = "user", content = new object[] {
                    new { type = "input_text", text = "Describe only the visible content and photographic qualities of this image." },
                    new { type = "input_image", image_url = "data:image/jpeg;base64," + Convert.ToBase64String(bytes), detail = "high" } } } },
                text = new { format = ReferenceAnalysisResponseFormat.Create() }
            });
            ...
            return result is null ? throw new ProviderFailureException(ProviderFailureKind.InvalidOutput)
                : JsonSerializer.Deserialize<ReferenceAnalysisResult>(result, Json) ?? throw new ProviderFailureException(ProviderFailureKind.InvalidOutput);
        }
