...
public sealed class OpenAiCritiqueProvider(IHttpClientFactory clients, IImageStore images, IOptions<AiOptions> options, TimeProvider clock) : ICritiqueProvider
{
    ...
    public async Task<CritiqueResult> GenerateAsync(CritiqueInput input, AnalysisIdentity identity, CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(options.Value.ApiKey)) throw new ProviderFailureException(ProviderFailureKind.InvalidCredentials);
