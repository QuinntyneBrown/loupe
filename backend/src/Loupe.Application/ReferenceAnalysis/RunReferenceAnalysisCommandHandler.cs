...
public sealed class RunReferenceAnalysisCommandHandler(IReferenceAnalysisWorkStore work, IReferenceAnalysisProvider provider, IReferenceAnalysisConfiguration configuration, IAnalysisFailureStore failures, TimeProvider clock) : IRequestHandler<RunReferenceAnalysisCommand, bool>
{
    public async Task<bool> Handle(RunReferenceAnalysisCommand request, CancellationToken cancellationToken)
    {
        ...
        ReferenceAnalysisResult? result = null;
        var timedOut = false;
        ...
        else if (result is not null && ReferenceAnalysisResultValidator.IsValid(result)) await work.PublishAsync(operation, result, cancellationToken);
