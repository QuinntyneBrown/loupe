...
public sealed class UpdateLocationTextCommandHandler(ICurrentOwner owner, ILocationStore locations, IEmbeddingConfiguration embeddings) : IRequestHandler<UpdateLocationTextCommand, LocationResult>
{
    public async Task<LocationResult> Handle(UpdateLocationTextCommand request, CancellationToken cancellationToken)
    {
        ...
        return LocationResult.From(await locations.UpdateTextAsync(request.Id, owner.Id, request.Revision, request.Field, text, cancellationToken), embeddings.IsConfigured);
    }
}
