...
public sealed class UpdateLocationCommandHandler(ICurrentOwner owner, ILocationStore locations, IEmbeddingConfiguration embeddings) : IRequestHandler<UpdateLocationCommand, LocationResult>
{
    public async Task<LocationResult> Handle(UpdateLocationCommand request, CancellationToken cancellationToken)
    {
        ...
        return LocationResult.From(await locations.UpdateAsync(request.Id, owner.Id, request.Revision, details, cancellationToken), embeddings.IsConfigured);
    }
}
