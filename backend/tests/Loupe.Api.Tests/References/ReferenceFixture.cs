...
{
    public static async Task<HttpResponseMessage> SubmitAsync(HttpClient client, IReadOnlyDictionary<string, string>? fields = null, string? key = null, string path = "/api/references/images", HttpMethod? method = null)
    {
        ...
        using var request = new HttpRequestMessage(method ?? HttpMethod.Post, path) { Content = upload };
