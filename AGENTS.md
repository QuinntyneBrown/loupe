...
Loupe is a web platform for developing photography skills through actionable AI
photo critiques and a curated inspiration library of images and photographer
websites, organized with boards, AI tags, and semantic search.
...
- Folders and namespaces agree. Controllers live in a `Controllers` folder and are
  namespaced `Loupe.Api.Controllers`.
...
- `IQuoteService` declares the contract and `QUOTE_SERVICE` is its `InjectionToken`;
  both live in `quote.service.contract.ts`. Name interface files
  `<entity>.service.contract.ts` and place the corresponding injection token in
  that same file. Each implementation lives in a separate file.
