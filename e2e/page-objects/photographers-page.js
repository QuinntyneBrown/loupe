...
export class PhotographersPage {
  ...
  async openAdd() {await this.page.getByRole('button',{name:'Add photographer',exact:true}).first().click();}
  async readPortfolio(url,name='') {await this.dialog().getByRole('textbox',{name:'Website',exact:true}).fill(url); if(name) await this.dialog().getByRole('textbox',{name:/^Name/}).fill(name); await this.dialog().getByRole('button',{name:'Read the page',exact:true}).click();}
  async expectReading() {await expect(this.dialog().getByRole('heading',{name:'Reading the page…',exact:true})).toBeVisible();}
  async expectPreview() {await expect(this.dialog().getByRole('textbox',{name:/^Description/})).toBeVisible();}
  async editPreview(name,description,notes) {await this.dialog().getByRole('textbox',{name:'Name',exact:true}).fill(name); await this.dialog().getByRole('textbox',{name:/^Description/}).fill(description); await this.dialog().getByRole('textbox',{name:/^Your notes/}).fill(notes);}
  async addTag(name) {const input=this.dialog().getByRole('textbox',{name:/^Tags/}); await input.fill(name); await input.press('Enter');}
  async removeTag(name) {await this.dialog().getByRole('button',{name:`Remove tag ${name}`,exact:true}).click();}
  async saveAdd() {await this.dialog().getByRole('button',{name:'Add photographer',exact:true}).click();}
  async cancelAdd() {await this.dialog().getByRole('button',{name:'Cancel',exact:true}).click(); await expect(this.dialog()).not.toBeVisible();}
  async expectClosed() {await expect(this.dialog()).not.toBeVisible();}
  async expectFallback() {await expect(this.dialog().getByRole('heading',{name:"Couldn't read that site",exact:true})).toBeVisible();}
  async manualFallback(name,notes) {await this.dialog().getByRole('textbox',{name:'Name',exact:true}).fill(name); await this.dialog().getByRole('textbox',{name:/^Your notes/}).fill(notes);}
  async expectDuplicate() {await expect(this.dialog().getByRole('heading',{name:'Already bookmarked',exact:true})).toBeVisible();}
  async expectSaveFailure() {await expect(this.dialog().getByRole('alert')).toContainText("Couldn't add this photographer");}
  async retryAdd() {await this.dialog().getByRole('button',{name:'Try again',exact:true}).click();}
