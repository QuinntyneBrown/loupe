...
test('L2-008.2: save first, then admit critique and open its durable status', async ({ page }) => {
  ...
  await upload.viewCompletedPhotograph(); await detail.expectImage('Morning'); await detail.expectCritiqueStatus('Queued', 'Waiting to start.');
...
test('L2-008.2/L2-030: a lost admission reply retries the same request without another upload', async ({ page }) => {
  ...
  await upload.retryCritique(); await upload.viewCompletedPhotograph(); await detail.expectImage('Morning'); await detail.expectCritiqueStatus('Queued', 'Waiting to start.');
...
test('L2-008.2/L2-043: failed upload retains the critique choice and requests only after upload acknowledgment', async ({ page }) => {
  ...
  expect(work.library.calls).not.toContain('requestCritique');
  await upload.retry(); await upload.viewCompletedPhotograph(); await detail.expectImage('Morning'); await detail.expectCritiqueStatus('Queued', 'Waiting to start.');
  expect(work.library.photos).toHaveLength(1);
