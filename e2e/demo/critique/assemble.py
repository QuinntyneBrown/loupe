...
record=json.loads((SRC/'recording.json').read_text())
narration={s['id']:s for s in json.loads((SRC/'narration.json').read_text())}
assert record['errors']==[] and record['unexpectedRequests']==[] and record['critiqueRequests']==0 and record['notesSaved']
...
for index,scene in enumerate(record['timeline']):
    ...
    ass+=event(0,duration,'Badge',r'{\pos(1416,22)}FIXTURE CRITIQUES  /  NO ANALYSIS CALLS')
...
run(['-y','-f','concat','-safe','0','-i','concat.txt','-c','copy','-movflags','+faststart',str(OUT/'critique-tour.mp4')])
...
(OUT/'transcript.md').write_text('# Critique page tour\n\nThis recording uses fabricated critique fixtures in the real Angular interface. No critique-provider calls were made. Narration is synthetic (Microsoft Edge Emma Multilingual). Production Demo execution remains removed.\n\n'+''.join(f"## {timestamp(c['start'],'.')[:8]} — {c['title']}\n\n{c['text']}\n\n" for c in chapters)+'Reference: `docs/mocks/critique.html`. The shoreline image is the same public placeholder used by the mock: https://picsum.photos/seed/lp-s3/1200/800. The second attempt is a fixture using that same photograph, not a genuine reshoot.\n',encoding='utf-8')
run(['-y','-ss','15','-i',str(OUT/'critique-tour.mp4'),'-frames:v','1',str(OUT/'poster.jpg')])
